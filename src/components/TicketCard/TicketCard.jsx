  ...
  return (
    <div className={styles['ticket-card']}>
      <div className={styles['ticket-card__header']}>
        {ticket.price} Р
        <img src={`https://pics.avs.io/99/36/${ticket.carrier}.png`} alt={`Лого ${ticket.carrier}`} />
      </div>
      <div className={styles['ticket-card__body']}>
        {ticket.segments.map((segment, index) => (
          <div key={index} className={styles['ticket-card__body-info']}>
            ...
            <div className={styles['ticket-card__body-info-text']}>
              <span className={styles['ticket-card__body-info-text-top']}>В ПУТИ</span>
              <span className={styles['ticket-card__body-info-text-bottom']}>{formatTime(segment.duration)}</span>
            </div>

            <div className={styles['ticket-card__body-info-text']}>
              <span className={styles['ticket-card__body-info-text-top']}>{formatStops(segment.stops)}</span>
              <span className={styles['ticket-card__body-info-text-bottom']}>{segment.stops.join(', ')}</span>
            </div>
          </div>
        ))}
      </div>
